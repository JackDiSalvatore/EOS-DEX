  ...
  >
    ...
    <v-row>
      ...
      <v-col
        ...
        class=""
      >
        {{ parseFloat( Math.ceil(order.price * order.amount * 100)/100 ).toFixed(2) }}
      </v-col>
      <v-col
        cols="2"
        align="start"
    ...
    <v-row>
      ...
      >
        <v-btn
          ...
          @click="buyOrder"
        >
          <span class="buyButtonText">Buy</span>
        </v-btn>
      </v-col>
    </v-row>
...
export default {
  name: 'PlaceBuyOrder',
  props: ['endpoint', 'privateKey', 'market', 'quote', 'account'],
  components: {
  },
  data: () => ({
    api: null,
        ...
        var tx_actions = [
            // place trade
            {
              ...
              data: {
                ...
                order_type: true,
                price: {
                  quantity: this.order.price + ' ' + this.priceLabel.symbol,
