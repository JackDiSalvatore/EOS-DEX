  ...
  <div v-if="accountBids[0]">
    <v-simple-table dense height="180px">
      <template v-slot:default>
        ...
        <tbody>
            <tr v-for="item in accountBids" :key="item.id">
            <td>{{ item.id }}</td>
...
export default {
  ...
  computed: {
    accountBids: function () { return this.bids.filter(row => row.trader == this.account) },
  },
  watch: {
