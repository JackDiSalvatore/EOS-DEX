  ...
  <v-card outlined tile class="px-3">
    ...
    <v-row>
      <v-col cols="4" align="start" justify="center">
        <h3 class="accountName">Account</h3>
      </v-col>
  ...
  methods: {
    refresh () {
      ...
      this.interval = setInterval(function () {
        self.getBaseBalance(self.market.value.contract, self.accountName)
        self.getQuoteBalance(self.quote.contract, self.accountName)
      }, 1000);
    },
    getQuoteBalance (contract_account, user_account) {
      ...
      .then( (response) => {
        ...
        for(let i = 0; i < this.quoteAccounts.length; i++) {
          if(this.quoteAccounts[i].balance.replace(/\d+./g, '') === this.quote.symbol) {
            this.userQuoteBalance = this.quoteAccounts[i].balance
          }
        }
      })
      .catch( (error) => {
    ...
    getBaseBalance (contract_account, user_account) {
      axios.post(this.$store.getters.endpoint.endpoint + '/v1/chain/get_table_rows', {
        code: contract_account,
        scope: user_account,
        table: 'accounts',
      ...
      .then( (response) => {
        ...
        for(let i = 0; i < this.baseAccounts.length; i++) {
          if(this.baseAccounts[i].balance.replace(/\d+./g, '') === this.market.value.quantity.replace(/\d+./g, '')) {
            this.userBaseBalance = this.baseAccounts[i].balance
          }
        }
      })
      .catch( (error) => {
